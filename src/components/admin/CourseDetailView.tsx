...
export default function CourseDetailView({ courseId, onBack, onUpdated }: Props) {
  ...
  async function loadCourse() {
    ...
    if (courseData) { setCourse(courseData); setTitleValue(courseData.title); setWeekCountInput(courseData.total_weeks.toString()); }
    const { data: weeksData } = await supabase.from("course_weeks").select("*").eq("course_id", courseId).order("week_number", { ascending: true });
  ...
  async function handleSaveTitle() {
    ...
    setCourse({ ...course, title: titleValue.trim() }); setEditingTitle(false); onUpdated();
  }
  ...
  async function handleSetWeeks() {
    ...
    for (let i = 1; i <= count; i++) {
      if (!weeks.find((w) => w.week_number === i)) {
        await supabase.from("course_weeks").insert({ course_id: course.id, week_number: i, title: `Week ${i}`, scheme_of_work: "", resources: "", status: "not_started" });
      }
    }
    await loadCourse(); setSettingWeeks(false);
  }

  const weekFieldMap: Record<string, keyof CourseWeek> = { title: "title", scheme: "scheme_of_work", resources: "resources" };
  ...
  async function handleSaveWeek() {
    ...
    await supabase.from("course_weeks").update({ [dbField]: editValue }).eq("course_id", courseId).eq("week_number", num);
    setWeeks((prev) => prev.map((w) => (w.week_number === num ? { ...w, [dbField]: editValue } : w)));
    setEditingWeek(null); setSavingWeek(false);
  }

  async function handleStatusToggle(weekNum: number, currentStatus: string) {
    const next = currentStatus === "not_started" ? "in_progress" : currentStatus === "in_progress" ? "completed" : "not_started";
    await supabase.from("course_weeks").update({ status: next }).eq("course_id", courseId).eq("week_number", weekNum);
    setWeeks((prev) => prev.map((w) => (w.week_number === weekNum ? { ...w, status: next } : w)));
  }
  ...
  if (loading) return <div className="flex items-center justify-center py-24"><Loader2 className="w-6 h-6 text-[#86868b] animate-spin" /></div>;
  if (!course) return <div className="text-center py-24 text-[15px] text-[#86868b]">Course not found</div>;
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="rounded-[16px] border border-[#e5e5ea] dark:border-[#38383a] bg-white dark:bg-[#1c1c1e] p-6">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            {editingTitle ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setEditingTitle(true)} className="w-8 h-8 rounded-[8px] flex items-center justify-center text-[#86868b] hover:bg-[#f2f2f7] dark:hover:bg-[#2c2c2e] transition-colors"><Pencil className="w-3.5 h-3.5" /></button>
              </div>
            )}
            <div className="flex items-center gap-3 mt-2">
              <span className="text-[13px] text-[#86868b] dark:text-[#98989d]">Created {new Date(course.created_at).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</span>
          ...
          <button onClick={handleDelete} className="w-9 h-9 rounded-[10px] flex items-center justify-center text-[#86868b] hover:bg-[#ff453a]/10 hover:text-[#ff453a] transition-colors"><Trash2 className="w-4 h-4" /></button>
        </div>
      </div>
                    ...
                    <div>
                      ...
                      {editingWeek?.num === week.week_number && editingWeek.field === "title" ? (
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => startEditWeek(week.week_number, "title")} className="w-6 h-6 rounded-[6px] flex items-center justify-center text-[#86868b] hover:bg-[#f2f2f7] dark:hover:bg-[#2c2c2e]"><Pencil className="w-3 h-3" /></button>
                        </div>
                      )}
                    </div>
                    {/* Scheme of Work */}
                    <div>
                      <label className="block text-[11px] font-medium text-[#86868b] uppercase tracking-wider mb-1.5">Scheme of Work</label>
                      {editingWeek?.num === week.week_number && editingWeek.field === "scheme" ? (
                        ...
                        <div className="group flex items-start gap-2">
                          <p className={`text-[13px] leading-relaxed ${week.scheme_of_work ? "text-[#1d1d1f] dark:text-white" : "text-[#86868b] italic"}`}>{week.scheme_of_work || "No scheme of work added yet."}</p>
                          <button onClick={() => startEditWeek(week.week_number, "scheme")} className="w-6 h-6 rounded-[6px] flex items-center justify-center text-[#86868b] hover:bg-[#f2f2f7] dark:hover:bg-[#2c2c2e] shrink-0 mt-0.5"><Pencil className="w-3 h-3" /></button>
                        </div>
                      )}
                    </div>
                    {/* Resources */}
                    <div>
                      <label className="block text-[11px] font-medium text-[#86868b] uppercase tracking-wider mb-1.5">Resources</label>
                      {editingWeek?.num === week.week_number && editingWeek.field === "resources" ? (
                        ...
                        <div className="group flex items-start gap-2">
                          <p className={`text-[13px] leading-relaxed ${week.resources ? "text-[#1d1d1f] dark:text-white" : "text-[#86868b] italic"}`}>{week.resources || "No resources added yet."}</p>
                          <button onClick={() => startEditWeek(week.week_number, "resources")} className="w-6 h-6 rounded-[6px] flex items-center justify-center text-[#86868b] hover:bg-[#f2f2f7] dark:hover:bg-[#2c2c2e] shrink-0 mt-0.5"><Pencil className="w-3 h-3" /></button>
                        </div>
                      )}
                    </div>
                  </div>
